...
class Recomend {
   public:
    ...
    std::vector<std::pair<std::string, float>> getRecommendedItems(std::unordered_map<std::string, std::unordered_map<std::string, float>> prefs, std::unordered_map<std::string, std::vector<std::pair<std::string, float>>> itemSim, std::string user);
};
